...
export async function createOrderSellOnt(amountOfOntToSell, amountOfEthToBuy, secret, initiatorAddress) {
  try {
    ...
    let gasPrice = 0;
    let gasLimit = 300000;
    let result = await client.api.smartContract.invoke({ scriptHash, operation, args, gasPrice, gasLimit });
    console.log(result);
  } catch (e) {
    console.log(e);
  }
...
export async function get_initiator(secret) {
  try {
    ...
    let args = [
      { type: 'String', value: hashlock },
    ];
    let result = await client.api.smartContract.invokeRead({ scriptHash, operation, args });
